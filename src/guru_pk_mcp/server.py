...
**状态**: {'✅ 已完成' if status['is_completed'] else '🔄 进行中'}"""
    ...
    async def _handle_list_available_personas(
        ...
        try:
            ...
            result += "💡 **使用提示**: 推荐使用智能专家推荐，让MCP Host端LLM为您智能选择最合适的专家组合！\n\n"
            result += "🤖 **智能推荐流程**:\n"
            result += '1. 获取推荐: `get_smart_recommendation_guidance({"question": "你的问题"})`\n'
            result += '2. 启动会话: `start_pk_session({"question": "你的问题", "personas": ["推荐专家1", "推荐专家2", "推荐专家3"], "recommended_by_host": true})`\n\n'
            result += "🔄 **手动选择**: 如需手动指定专家，可直接使用专家名称启动会话\n"
